...
function buildMissingModal() {
  ...
  const { total, collected, percent, byCountry } = collectionStore.getProgress(countries);
...
function buildCreditsModal() {
      ...
      <div class="fa-modal-section">
        ...
        <div class="fa-modal-share-row">
          <a class="fa-modal-share-btn fa-share-wa"
             href="https://wa.me/?text=${encodeURIComponent(t('credits_share_text') + ' ' + ALBUM_URL)}"
             target="_blank" rel="noopener">
          ...
          <button class="fa-modal-share-btn fa-share-copy" data-url="${ALBUM_URL}">
  ...
  overlay.querySelector('.fa-share-copy').addEventListener('click', (btn) => {
    navigator.clipboard?.writeText(ALBUM_URL).then(() => {
...
function buildMenuModal({ es, onPack, onPackTick }) {
  ...
  const { percent, collected, total } = collectionStore.getProgress(countries);
  const pct = percent.toFixed(1);
...
export function createFloatingActions() {
  ...
  // ── Barra de progreso ──────────────────────────────────────────────────────
  const { percent } = collectionStore.getProgress(countries);
  const pct = percent.toFixed(1);
